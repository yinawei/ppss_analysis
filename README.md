...
(3). Basic paraview visualization workflow
   * Launch paraview (For Linux system, open the folder bin, in the terminal: ./paraview)
   * Right-click on builtin in the Pipeline Browser, then import /data/hippo/root.vtk or /data/hippo/CA1.vtk
   * Click the eye icon next to the imported VTK object in the Pipeline Browser to display it. 
...
uncheck Use Color Palette For Background and select white as the background color.
   * Click the View button in the toolbar and check Python Shell to display the script dialog
   * Click Run Script in the bottom-right corner and select the corresponding Python script file in the file explorer
